...
[ -n "$open" ] || exit 0
[ "$open" = "0" ] && { echo "- maintainer: no open issues — tracker clear."; exit 0; }
...
msg="- maintainer: $open open issue(s)"
[ "${p1:-0}" != "0" ] && msg="$msg — ${p1} P1"
[ "${doctrine:-0}" != "0" ] && msg="$msg, ${doctrine} incorrect-doctrine"
echo "$msg. Run /maintainer-triage to queue, /maintainer-work to ship."
exit 0
