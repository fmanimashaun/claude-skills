...
| [references/page-anatomies.md](references/page-anatomies.md) | **A whole screen.** 3 shell archetypes (sidebar + mobile drawer, stacked, multi-column) × 3 anatomies (home/dashboard, detail, settings), each with mobile behaviour and scroll containment; the data table's toolbar order and **selection and bulk actions**; the primitive-instead-of-breakpoint substitution table. A screen is composed, not designed. |
| [references/components.md](references/components.md) | **A specific component** — the catalog: each entry a composition of primitives + roles with a fixed variant × size × state vocabulary, a11y checklist and responsive behaviour — including the **permissions matrix**, the **period selector** and a **background operation** that outlives the request. Open the one entry you need; do not read the file through. |
...
| [references/forms.md](references/forms.md) | **Any form** — controls, simple_form wiring, validation display, the Turbo 422 contract, the file-upload **flow** after selection, and leaving a dirty form |
...
| [references/interaction-stimulus.md](references/interaction-stimulus.md) | **Behaviour** — the four reusable Stimulus mixins (list-navigation, focus-trap + restore, dismissable-layer, anchored-position) that cover every overlay; the power-user keyboard layer and its WCAG 2.1.4 obligation; styling off `data-[state=…]` / `aria-*` |
...
| [references/responsive.md](references/responsive.md) | **Any screen, small or very large** — fluid-first + intrinsic primitives, when a breakpoint is justified, the adaptive position and the size bands, which navigation belongs at which width, what happens past 1536px, `min-h-touch` 44px targets, safe-areas |
