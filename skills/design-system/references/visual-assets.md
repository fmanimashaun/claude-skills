...
## The library is curated once, then topped up

Assets are **not** commissioned one at a time as surfaces come up. That produces a pile rather than a
set — each piece defensible alone, the family incoherent — which is the look the rest of this file
exists to prevent.

The set is seeded **after the PRD becomes skills and the product intent is settled, before any
coding**, so it can be curated against what the product actually is. It lives in `docs/assets/` with
a manifest that records, per asset: purpose, use cases, **where it must not go**, visual elements,
style, kind (static / vector / motion) and surface. That manifest is a reference table an agent reads
to answer *may I put this here* — a row naming only a file gets the asset re-generated by whoever
could not tell.

Afterwards the library grows by **coverage, never duplication**: an agent that hits an uncovered
surface records the miss, generates one asset, and appends it to the same manifest.

`/design-flow:generate` enforces this — it refuses a request that never searched the library, and
refuses one whose surface and kind the manifest already lists.
