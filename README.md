...
| **`fidara-design`** | The Fidara design system: Tailwind v4 `@theme` token architecture (brand primitives → semantic roles → Utopia fluid scale), Every-Layout composition primitives, a ~16-component catalog (variant×size×state), Stimulus interaction patterns, responsive doctrine, mobile (Hotwire Native) parity, reference implementations (ViewComponent + Stimulus mixin + Hotwire-Native code), and the two-brand model — so UI is consistent across projects without a designer/Figma | 10 reference files |
