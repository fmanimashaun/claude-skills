...
`docs/assets/manifest.json` — at the assets-dir root, one level above the artefacts it describes.
The root holds the **indexes** (`plan.json`, `plan.md`, `manifest.json`); the two subfolders hold the
**contents** (`assets-library/`, `prompts-library/`). It exists to answer the question an agent
...
```json
{
  "assets": [
    {
      "file": "docs/assets/assets-library/hero-lattice.svg",
      "name": "Hero lattice",
...
| file | what it is |
|---|---|
| `docs/assets/prompts-library/prompts.json` | the source — the prompt verbatim, the model, both costs, the verdict |
| `docs/assets/prompts-library/prompts.md` | a **generated** view of it, for the human deciding whether to reuse or re-buy |
...
**The model may say `unknown`, and that is the honest answer.** The `agent` rung names
