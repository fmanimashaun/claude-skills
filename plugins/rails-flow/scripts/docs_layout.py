...
CANVAS_MODULE_EXT = {".js", ".mjs"}
CANVAS_IMPORT = re.compile(r"""(?:import\s*\(\s*|from\s+)['"]\./([\w.\- ]+\.m?js)['"]""")


def imported_by_sibling_canvas(p: Path) -> bool:
    """True when a `.dc.html` in the same directory imports this module relatively."""
    for canvas in p.parent.glob("*.dc.html"):
        try:
            text = canvas.read_text(encoding="utf-8", errors="replace")
        except OSError:
            continue
        if p.name in CANVAS_IMPORT.findall(text):
            return True
    return False
...
def kind_of(p: Path, docs: Path) -> str:
    ...
    if p.suffix.lower() in CODE_EXT:
        # A module a Claude Design canvas beside it imports (`import('./admin-core.js')`) is the design,
        # not code that wandered in: design-flow's `canvas_manifest.py extract` reads the canvas's literals
        # through that very import (#935), so the file has to stay where the canvas can find it (#942).
        if p.suffix.lower() in CANVAS_MODULE_EXT and imported_by_sibling_canvas(p):
            return "authored"
        return "code"
...
def relink_markdown(text: str, old_key: str, new_key: str, moved_to: dict[str, str], dirs: dict[str, str]) -> tuple[str, list[str]]:
    ...
    def sub(mm):
        ...
        # NOTHING MOVED FOR THIS LINK: leave it exactly as written (#1189). `relpath` normalises, so
        # recomputing an unchanged link turns `[F-29](./F-29-x.md)` into `[F-29](F-29-x.md)` -- a rewrite
        # with no move behind it. Measured downstream: asked to move three files, `--write` also rewrote
        # 32 unrelated feature docs this way, the tool's own link style applied to files nobody named.
...
def selftest() -> int:
    ...
    def make_retask_like(root: Path) -> None:
        ...
        for rel, text in {
            ...
            # #1189: an UNMOVED file whose `./` link targets an UNMOVED sibling. Nothing it points at moves,
            # so it must not be rewritten -- `relpath` would silently drop the `./`.
            "docs/brain/DECISIONS.md": "# Decisions\n\n- [status](./STATUS.md)\n", "docs/brain/MEMORY.md": "- [x](memos/feedback/x.md) — y\n",
            ...
            "docs/design/admin/Admin.dc.html": "<x-dc></x-dc><script>import('./admin-core.js').then(m => m);</script>\n",
            "docs/design/admin/admin-core.js": "export const TONE = {};\n",
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        check("a module a sibling canvas imports is the design, not code (#942)", rows["docs/design/admin/admin-core.js"]["kind"] == "authored", rows["docs/design/admin/admin-core.js"]["kind"])
        with tempfile.TemporaryDirectory() as td2:
            loose_root = Path(td2)
            (loose_root / "docs/design").mkdir(parents=True)
            (loose_root / "docs/design/loose.js").write_text("console.log('no canvas imports this');\n", encoding="utf-8")
            (loose_root / "docs/design/note.dc.html").write_text("<x-dc></x-dc>\n", encoding="utf-8")
            loose = {r["path"]: r for r in classify(loose_root)}
            check("a .js no canvas imports is still code, even beside a canvas", loose["docs/design/loose.js"]["kind"] == "code", loose["docs/design/loose.js"]["kind"])
        ...
        check("#1189: a file whose links point at nothing that moved is not rewritten at all (./ kept)",
              "docs/brain/DECISIONS.md" not in p["rewrites"] and not any(m["path"] == "docs/brain/DECISIONS.md" for m in p["moves"]),
              p["rewrites"].get("docs/brain/DECISIONS.md", "")[:160])
