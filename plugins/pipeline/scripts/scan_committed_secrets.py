...
def _routed(text: str):
    """(key, raw value) for each assignment in a secret-routed section, in file order."""
    secret = True  # before any section tag: treat as secret
...
def routed_names(text: str) -> list[str]:
    """NAMES of the secret-routed keys that hold a value. Only names ever reach the output; this
    function returns no value, so none can be printed by accident."""
    return sorted(key for key, raw in _routed(text) if _clean(raw))


def routed_values(text: str) -> dict[str, str]:
    """KEY -> value, for comparison only. Nothing built from these values is printed."""
    return {key: _clean(raw) for key, raw in _routed(text) if _clean(raw)}
...
def scan(root: Path) -> tuple[int, list[str]]:
    ...
    long_names = [k for k in names if len(routed_values(text_env)[k]) >= MIN_LEN]
    count_scanned = len(long_names)
    ...
    for path in files:
        ...
        hits = [k for k in long_names if routed_values(text_env)[k] in text]
        for key_name in hits:
            findings.append(f"  [secret-in-committable-file] {key_name} appears in {path.relative_to(root)}")
    ...
    return 0, [f"no secret from {BRIEFING} in any of {len(files)} committable file(s); "
               f"{count_scanned} value(s) scanned{note}"]
...
def selftest() -> int:
    ...
    expect("an untagged key counts as a secret", "X" in routed_names("X=abcdefghij\n"))
